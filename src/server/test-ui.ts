...
const root = (import.meta.url.split('file:').pop() as string).split('dist').shift() as string

export default function TestUi(path: string, stream: ServerHttp2Stream) {
    ...
    if (path === 'data') {
        ...
        try { fileData = readFileSync(join(Config.testResultsDirectory, 'results.json')).toString() } catch (error) { }
